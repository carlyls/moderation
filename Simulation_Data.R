...
DataGen <- function(n.trial, n.obs, reg.coefs, mu.trial, mu.obs,
  ...
  lin_pred <- -1/2 + X1.obs/2 + .4*X2.obs + .4*X2.obs^2
  ...
  for(k in 1:n.trial) {
    dd <- c(1, A.trial[k], X1.trial[k], X2.trial[k], X3.trial[k], (X1.trial[k])^2, A.trial[k]*X2.trial[k], A.trial[k]*X3.trial[k])
    lin_pred <- sum(reg.coefs*dd)
    outcome.prob.trial[k] <- exp(lin_pred)/(1 + exp(lin_pred))
  }
  for(k in 1:n.obs) {
    dd <- c(1, A.obs[k], X1.obs[k], X2.obs[k], X3.obs[k], (X1.obs[k])^2, A.obs[k]*X2.obs[k], A.obs[k]*X3.obs[k])
    lin_pred <- sum(reg.coefs*dd)
